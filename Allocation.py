...
def check_details(value):
    ...
    if state and value[1] == "driver_age" and value[2] < 1:
        return True
...
while True:
    ...
    if operation == "Create_parking_lot":
        ...
        if not check_details(value):
            continue
        parking_lot.park_the_vehicle(value[0], value[2])
    elif operation == "Slot_numbers_for_driver_of_age":
        parking_lot.get_slots_by_driver_age(value[0])
    elif operation == "Slot_number_for_car_with_number":
        parking_lot.get_vehicle_number_by_driver_age(value[0])
    elif operation == "Leave":
        parking_lot.leave(value[0])
    elif operation == "Vehicle_registration_number_for_driver_of_age":
        parking_lot.get_slot_number_by_vehicle_number(value[0])
    else:
